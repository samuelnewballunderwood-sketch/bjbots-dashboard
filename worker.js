...
// ── WRITE ENDPOINTS (Layer 1: Action Execution) ───────────────────────────────
...
// ── SCORING ENGINE (Layer 2: Intelligence) ────────────────────────────────────
// Runs server-side so decisions are consistent and auditable

function scoreBot({ roi, trades, drawdownPct, change24h, type }) {
  // ROI score (30%) — normalized around realistic returns
  const roiScore     = Math.min(100, Math.max(0, 50 + roi * 15));
  // Drawdown score (25%) — penalises losses
  const ddScore      = Math.min(100, Math.max(0, 100 - drawdownPct * 4));
  // Activity score (15%) — rewards consistent execution
  const actScore     = Math.min(100, trades * 8);
  // Consistency score (15%)
  const conScore     = trades > 5 ? 80 : trades > 0 ? 40 + trades * 8 : 15;
  // Market fit score (15%) — grid bots score higher in sideways, DCA in trend
  const absChange    = Math.abs(change24h);
  const marketFit    = type === 'spot-grid' || type === 'futures-grid'
    ? (absChange < 2 ? 80 : absChange < 4 ? 55 : 35)   // grids like low vol
    : (absChange > 1 ? 75 : 45);                         // DCA/signal like trend
...
function decisionEngine({ bots, tcBots, floatingPnl, longPct, market }) {
  ...
  // ── 1. Signal bots — always check execution rate ──────────────────────────
  ...
  // ── 2. XRP grid — negative ROI, low trades ────────────────────────────────
  const xrp = bots.find(b => b.id === 'xrp-grid-trades');
  if (xrp && xrp.trades < 3) {
    decisions.push({
      action: 'REVIEW', type: 'reduce',
  ...
  // ── 3. Hedge scaling — long bias detection ────────────────────────────────
  if (longPct > 65) {
    const hedgeBot = tcBots.find(b => b.id === 16801248);
    decisions.push({
      action: 'INCREASE', type: 'increase',
  ...
  // ── 6. Floating loss warning ──────────────────────────────────────────────
  if (floatingPnl < -10) {
    decisions.push({
      action: 'ALERT', type: 'pause',
  ...
  // ── 7. Market regime — grid suitability ──────────────────────────────────
...
async function getDecisions(env) {
  try {
    ...
    // Also compute scores for all bots
    const scores = {};
    const tcBots = tcData.bots || [];
    const bnBots = bnData.bots || [];

    tcBots.forEach(b => {
      const capital = { 16801943:350, 16801248:250, 16801317:100, 16801290:100, 194116:100, 194115:100 }[b.id] || 100;
      const roi     = b.profit ? (b.profit / capital) * 100 : 0;
      const trades  = (b.completedDeals||0) + (b.activeDeals||0);
      scores[b.id]  = scoreBot({ roi, trades, drawdownPct: roi < 0 ? Math.abs(roi) : 0, change24h: bnData.market?.btcChange24h || 0, type: 'dca' });
    });
    ...
    bnBots.forEach(b => {
      ...
      const m = META[b.id];
      if (m) scores[b.id] = scoreBot({ roi: m.roi, trades: b.trades, drawdownPct: m.roi<0?Math.abs(m.roi):0, change24h: b.change24h||0, type: m.type });
    });
...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      // WRITE — bot actions (require POST)
      if (path.startsWith('/api/bot/') && request.method === 'POST') {
        const parts  = path.split('/'); // ['', 'api', 'bot', ':id', ':action']
        const botId  = parts[3];
